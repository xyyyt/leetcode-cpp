...
public :
    int uniquePathsIII(std::vector<std::vector<int>> grid)
    {
        ...
        const int totalEmptySquare = getTotalEmptySquare(grid);
...
private :
    ...
    inline int getTotalEmptySquare(
        const std::vector<std::vector<int>>& grid) const noexcept
    {
        ...
        int posX) const
    {
        if (grid[posY][posX] == to(Square::ENDING))
        {
            ...
            // vcSet.size() - 1 because starting square doesn't count as a empty square
