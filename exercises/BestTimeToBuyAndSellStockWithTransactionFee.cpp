...
struct BestTimeToBuyAndSellStock
{
        ...
        const BestTimeToBuyAndSellStock& rhs)
    {
        ...
        auto fillVectorFrom = [](std::vector<std::pair<int, int>>& timesToBuyAndSell,
...
{
public :
    BestTimeToBuyAndSellStock maxProfit(std::vector<int> prices, int fee)
    {
...
private :
    [[nodiscard]]
    BestTimeToBuyAndSellStock maxProfit(const std::vector<int>& prices, int fee, int n)
    {
        ...
        while (n < prices.size())
        {
            ...
            for (int n2 = n + 1; n2 < prices.size(); ++n2)
            {
                ...
                {
                    BestTimeToBuyAndSellStock bttbass{
                        (prices[n2] - prices[n]) - fee, {{n, n2}}};
                    BestTimeToBuyAndSellStock subBTTBASS =
                        maxProfit(prices, fee, n2);
